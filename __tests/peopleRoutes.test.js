...
describe('/api/people routes', () => {
  describe('GET to /api/people', () => {
    ...
    // using async/await
    it('should filter users using the isAttending query string', async () => {
      try {
        // seed the db
        await Promise.all([
        ...
        const isNotAttendingResponse = await request(app)
          .get('/api/people/?isAttending=false')
  ...
  xdescribe('POST to /api/people', () => {
    ...
    it('should return status code 400 if missing required information', () => {
      // make sure the server didn't write anything to the db either!
    });
  });
